...
const SectionCourseComponent = (props : SectionCourseProps) => {
    ...
    useEffect(() => {
        ...
        setValidUnits(sem1Text + sem2Text + sumText ? sem1Text + sem2Text + sumText : "This course is not currently available");
    }, [props.course]);

    const onDragStart = (e : any, course: Course) => {
        e.dataTransfer.setData("code", course.code);
        e.dataTransfer.setData("title", course.title);
        e.dataTransfer.setData("units", course.units);
        e.dataTransfer.setData("sem1", course.sem1);
        e.dataTransfer.setData("sem2", course.sem2);
        e.dataTransfer.setData("sum", course.sum);
